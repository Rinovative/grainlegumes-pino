# ruff: noqa: S101
"""
Exercise scientific plot formulas and public analysis usability on schema-4 fixtures.
...
def _evaluation_frame(root: Path, *, role: str, bias: float) -> pd.DataFrame:
    """
    Create two schema-4 rows and their matching NPZ case payloads.
    ...
    return analysis.evaluation.dataframe.build_eval_df(raw)
...
def evaluation_datasets(tmp_path: Path) -> dict[str, pd.DataFrame]:
    ...
    while reusing the same complete schema-4 construction.
    """
    return {
...
def test_retired_continuity_names_are_absent_from_maintained_analysis() -> None:
    """
    Keep retired continuity aliases out of maintained plot modules.

    A source scan protects the explicit dual-continuity contract from accidental
    reintroduction of selected ``cont_mse`` or ambiguous ``Rc`` readers.
    """
    plot_root = Path(analysis.evaluation.plots.__file__).parent
    offenders: list[str] = []
    for path in plot_root.glob("*.py"):
        text = path.read_text(encoding="utf-8")
        if '"cont_mse"' in text or '"Rc"' in text:
