...
def build_batch_dataset(batch_name: str, verbose: bool = False) -> dict:
    """
    Builds structured .pt case files and a batch-level meta.pt file.
    ...
    if not common:
        raise RuntimeError(f"No matching case_XXXX.json / case_XXXX_sol.csv found in {batch_name}")

    log.append(f"Found {len(common)} matching case pairs.")

    # -------------------------- helper --------------------------
    def load_case(csv_path: Path, meta_path: Path):
        ...
        with open(csv_path, "r") as f:
            lines = f.readlines()
        header_line = [l for l in lines if l.strip().startswith("%")][-1]
